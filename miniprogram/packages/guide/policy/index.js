...
const POLICY_DEFINITIONS = {
  agreement: {
    title: "用户协议",
    fetcher: fetchLatestUserAgreement
  },
  privacy: {
    title: "隐私政策",
    fetcher: fetchLatestPrivacyPolicy
  }
};
...
function normalizeType(type) {
  ...
  if (key.includes("agreement") || key.includes("user")) return "agreement";
  return "agreement";
}
...
Page({
  data: {
    ...
    contentNodes: "",
    contentSegments: [],
    title: POLICY_DEFINITIONS.agreement.title,
    imageUrls: []
  },
  ...
  loadContent(options = {}) {
    ...
    const fetcher = this._policyFetcher || fetchLatestUserAgreement;
    fetcher({ apiBase })
      .then((payload = {}) => {
